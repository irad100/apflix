...
system_message = f"""You are a movie reccommendation bot, You should output JSON with the fields: "title", "reason".
...
def get_movie_data(title):
    try:
        response = requests.get(
            f"https://api.themoviedb.org/3/search/movie?api_key={os.environ['TMDB_API_KEY']}&query={title}"
        ).json()
